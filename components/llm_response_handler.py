...
class LLMResponseHandler:
    ...
    def create_fallback_response(self, use_click_probability: float = 0.2) -> Tuple[str, int]:
        ...
        if self.debug:
            logging.debug("Generated fallback response: %s, %ds", event_type, dwell_time)
    ...
    def _parse_single_response_for_all(self, response: Dict, content_id: int) -> Tuple[bool, int]:
        ...
        if not isinstance(clicked, bool):
            if self.debug:
                logging.warning("Invalid clicked value for %d: %s, using False", content_id, clicked)
            clicked = False
        ...
        if not isinstance(dwell_time, (int, float)) or dwell_time < 0:
            if self.debug:
                logging.warning("Invalid dwell_time for %d: %s, using 0", content_id, dwell_time)
            dwell_time = 0
        ...
        if not clicked and dwell_time > 0:
            if self.debug:
                logging.warning("Content %d: clicked=False but dwell_time=%d, correcting to 0", 
                              content_id, dwell_time)
            dwell_time = 0
        
        if self.debug:
            logging.debug("Parsed response for %d: clicked=%s, dwell_time=%ds", 
                         content_id, clicked, int(dwell_time))
    ...
    def _create_fallback_all_responses(self, all_contents: List[Dict]) -> List[Dict]:
        ...
        if self.debug:
            ...
            logging.debug("Generated fallback responses: %d total, %d clicked", 
                         len(responses), clicked_count)
